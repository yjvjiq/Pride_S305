    ...
    {            
        ...
        case 0xC01ee16:
            g_sysPara[PARA_WHOLE_CAPACITY]= *((&CAN2RXDSR0)+2);
	          g_ActalRatedCapty =  g_sysPara[PARA_WHOLE_CAPACITY];//可用容量初始化
	          StoreParameter();
	          StoreQ1value();
            break;
        ...
        case 0xC01EE2a:         //接收系统参数修正信息from 上位机
            ...
            break;
        case 0xC01EE2b:  
            //g_sysPara[PARA_BATTYPE]=*((&CAN2RXDSR0)+1);        //电池类型
            //StoreParameter();
            break; 
        case 0xC01EE8F:
            CellNumHigh=*((&CAN2RXDSR0)+0);   //单体数量
            g_sysPara[PARA_BATCELLNUMBER]= (CellNumHigh<<8)+(*((&CAN2RXDSR0)+1));   //单体数量    //接收系统参数修正信息from 上位机
            TotalVoltageOverVoltage_celln_P.Chart1_BatCellNumber=g_sysPara[PARA_BATCELLNUMBER];
            StoreParameter();
            break;
        case 0xC01EE9F:
            g_sysPara[PARA_BATTYPE]=*((&CAN2RXDSR0)+0);        //电池类型
            TotalVoltageOverVoltage_celln_P.Chart1_BatType=g_sysPara[PARA_BATTYPE];
            StoreParameter();
            break;
        default:
            break;                  
