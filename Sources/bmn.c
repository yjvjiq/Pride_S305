...
float g_BatSysTotalVolV1; //电池系统总电压
float g_BatSysTotalVolV2; //预充电继电器与预充电阻之间的电压
float g_BatSysTotalVolV3;//预充电继电器另一边的电压
...
void TaskVoltage(void)
{
    ...
    g_BatSysTotalVolV1 = (float)(Vpn1_Voltage_CAL_Value/100.0); // 内部电压 v1
    /////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////
    ...
    g_BatSysTotalVolV2 = (float)(Vpn2_Voltage_CAL_Value/100.0);//外部电压 v2    
    ...
    g_BatSysTotalVolV3 = (float)(Vpn3_Voltage_CAL_Value/100.0);//外部电压 v3
...
void TaskInsulation(void)//调用函数TaskInsulation(void)需要400ms;
{
    ...
    //if(Can10f2Byte0.Bit.st_ne_batteryRelay ==0)  //主负闭合后才可以检测绝缘
        //return;
    
    if((stateCode !=30) && (stateCode!=110) && (stateCode!=170)&&(stateCode!=179)) //只有在行车、快充。慢充三个状态中才检测绝缘   
        return;   
