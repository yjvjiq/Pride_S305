...
#include "BatteryTemperatureHigh.h"    /* Model's header file */
#include "rtwtypes.h"
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(BatteryTemperatureHigh_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  /* Initialize model */
  BatteryTemperatureHigh_initialize();
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(BatteryTemperatureHigh_M) == (NULL)) {
  ...
  /* Terminate model */
  BatteryTemperatureHigh_terminate();
  return 0;
}
