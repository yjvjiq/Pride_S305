...
float ChagerCurtCutDone(unsigned char ErrorLever,float CurrentValue,unsigned char Mode)
{
    ...
    if(Mode==1)
        CurrentChaCurrent = g_MaxFebCur;//该值不经过查表,显示当前的功率值
        //CurrentChaCurrent = MaxFebPower;//该值不经过查表,显示当前的功率值
    else if(Mode==2)
    ...
    {      
        ChargeCurrentOn=0.2*CurrentValue;
        ReturnDone=1;
    }
...
{   
    ...
    float ReturnValue=0;
    float feb=(float)tem;
    BigCharCurrent=Feedback176Ah_step(soc,feb);
    /*if(Tavg>0+40)
    { 
        if((BigCharCurrent*g_BatSysTotalVolV1/1000)>=100)
            BigCharCurrent=100000/g_BatSysTotalVolV1; 
    } 
    else if(Tavg>=(-15+40))
    {
        if((BigCharCurrent*g_BatSysTotalVolV1/1000)>=20)
            BigCharCurrent=20000/g_BatSysTotalVolV1; 
    } 
    ...
    if(ReduceCurt!=0)//有故障
    {
        if(ReduceCurt==1)
            ...
            BigChaPowerCutDown=0.2*BigCharCurrent;
...
//******************************************************************************
//* Function name:   Feedback_Continue
//* Description:     持续回馈电流查表
//* EntryParameter : tem, soc 平均温度，SOC值
//* ReturnValue    : fc 返回的回馈电流值  
//******************************************************************************
float Feedback_Continue(float soc,char tem)
{
    float fc=0;
    if((soc>=0.95)||(tem<=0)||(tem>=55))
        return 0;
    else
    {
        fc=C;
        if((soc<0.95)&&(soc>=0.8)&&(tem>0)&(tem<=10))
            fc=0; 
        return fc;
    }
}
...
{   
    ...
    BigCharCurrent=Feedback_Continue(soc,tem); 
