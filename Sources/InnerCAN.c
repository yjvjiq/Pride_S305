...
//----------------------------------------------------------------------------------------------------
#include "BMS20.h"
float  CC2Vol=0;//CC2电压
float  BMSLowVolPowerVol=0; //BMS低压电源电压
unsigned char ACCTestPlateTemp1;     //ACC检测板温度1
unsigned char ACCTestPlateTemp2;     //ACC检测板温度2
...
void bmsToPcInfo450(void)
{
    ...
    mg.data[6] = g_BatHighestTemp - g_BatLowestTemp;  //单体电池 温度差
    buff=(unsigned int)(BMSLowVolPowerVol*5);    //BMS低压电源电压;
    mg.data[7] =buff;      //BMS低压电源电压;
...
void bmsToPcInfo552(void)
{
    ...
    mg.data[0]= state_pc552_Byte0.byte;//C0552_0; //测试模式下A口的检测
    mg.data[1]= state_pc552_Byte1.byte;//BmsCtlStat0 & 0xff;//BMS控制状态
    mg.data[2]= (g_BatHighestTemp-40)+48;//电池最高温度 
	  mg.data[3]= (g_BatLowestTemp-40)+48;//电池最低温度 
	  //mg.data[2] = g_DCTemp1 + 48;
	  //mg.data[3] = g_DCTemp2 + 48;
    mg.data[4]= fault_pc552_Byte4.byte;//g_caution_Flag_1; //故障状态
...
void bmsToPcInfo553(void)
{
    ...
    AH = (unsigned char) g_ActalRatedCapty;//g_ActalRatedCapty;  //原始额定容量（真实SOC用),Q1
    mg.data[0] = AH; // gpio
    
    AH = (unsigned char) Q2;     //修正后额定容量（上报SOC用）
    mg.data[1]= AH;//
    
    buff = (unsigned int)(g_SysSOC*100*2.5);     //上报SOC
    mg.data[2] =(unsigned char)buff;// 电池荷电状态SOC 分辨率0.4,单位%
	  
	  buff=(unsigned int) (CC2Vol*5);         //CC2电压
	  mg.data[3]= buff; 
...
void bmsToPcInfo554(void)
{
    ...
    mg.data[4]= (g_DCTemp1+48);    //直流充电座温度1
    mg.data[5]= (g_DCTemp2+48);    //直流充电座温度2
    
    mg.data[6]= ACCTestPlateTemp1+48;     //ACC检测板温度1
    mg.data[7]= ACCTestPlateTemp2+48;     //ACC检测板温度2
