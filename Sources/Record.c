...
void StoreSysVariable(void)  //´æ´¢¹ÊÕÏÐÅÏ¢ºÍSOCÖµ£¬µØÖ·´Óat45db161 µÄ 0x3ff ¿ªÊ¼
{
    ...
    g_storageSysVariable[TOTAL_VOLTAGE]= (unsigned int)g_BatSysTotalVolV1;   
    g_storageSysVariable[TOTAL_CURRENT]= (unsigned int)((g_BatSysTotalCur+400)*50);
    g_storageSysVariable[PARA_SOC_DISPLAY]= (unsigned int)(g_socValue*100*2.5);
    g_storageSysVariable[SYS_CONTACTORS_STATE]= state_pc552_Byte1.byte & 0xff;//BMS¿ØÖÆ×´Ì¬ 
    //¹ÊÕÏÐÅÏ¢ÔÚ¹ÊÕÏ´¦Àí³ÌÐòÀï
    //ÊµÊ±Ê±¼ä
    for(j=SYS_REALTIME_SECOND;j<SYS_REALTIME_YEAR+1;j++)
        g_storageSysVariable[j]= CurrentTime[j-10];
    g_storageSysVariable[CELL_MAX_TEM] =(unsigned int)((g_BatHighestTemp-40)+48);
    g_storageSysVariable[CELL_MIN_TEM] = (unsigned int)((g_BatLowestTemp-40)+48);
    g_storageSysVariable[CELL_AVERAGE_TEM] = (unsigned int)((g_averageTemperature-40)+48);
    g_storageSysVariable[CELL_MAX_VOLTAGE] = (unsigned int)(g_CellHighestVol*10000);
    g_storageSysVariable[CELL_MIN_VOLTAGE] = (unsigned int)(g_CellLowestVol*10000);
    ...
    M95_Write_Memory(256*(unsigned long)2*g_errorCounter,(unsigned char* )buffer2_1,256);//´ÓµÚ1Ò³¿ªÊ¼£¨µØÖ·´Ó0¿ªÊ¼,0~255,´æ128¸öµ¥ÌåµçÑ¹
