...
void InitialBMS(void)
{
    unsigned char SetTime[7];
	  ...
	  TurnOn_CC2();//12v系统打开，24V关闭///////////////////////////////////////////////////////////////////////////////test///////////////////
	  
	  g_sysPara[PARA_BMU_NUMBER]=BMU_NUMBER;
	  g_sysPara[PARA_BATCELLNUMBER]=BATTERYCELLNUMBER;    //单体数量
	  g_sysPara[PARA_BATTYPE]=CELL_TYPE;       //电池类型
...
{		
    ...
    {      
        ...
        g_ActalRatedCapty= ReadOutQ1StoreValue();//读取Q1值
    }
    //当OCVState==1,Q1:g_ActalRatedCapty与g_socValue在OCV函数中赋值
    g_SysSOC = ReadOutCanSocStoreValue();//SOC显示值
    ...
    g_SysSOC_Start=g_SysSOC;
	  ...
	  Q2 = Q2nominalCalculate(g_ActalRatedCapty); //计算出额定容量
	  if(Q2>=g_ActalRatedCapty)
	      Q2=g_ActalRatedCapty;
	  g_leftEnergy = g_socValue*(g_ActalRatedCapty*3600);  //剩余容量(上电时的容量	  
	  g_energyOfUsed = 0;      
